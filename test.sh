...
docker run --rm --volume /tmp/debug-data:/data --volume `pwd`:/root --memory 512m --memory-swap 512m --cpu-shares 1024 --net bridge --env KBC_PARAMETER_DIRECTION=tables --env KBC_PARAMETER_ADDCSVSUFFIX=1 --env KBC_DATADIR=/data/ alpine:latest /bin/sh /root/direct-move.sh $SLICES_COUNT
